...
export class AppError extends HttpsError {
  constructor(code: string, message: string, status: FunctionsErrorCode = "invalid-argument") {
...
export const Errors = {
  unauthenticated: (msg = "Authentication required.") =>
    new AppError("UNAUTHENTICATED", msg, "unauthenticated"),
  forbidden: (msg = "You do not have permission to perform this action.") =>
    new AppError("FORBIDDEN", msg, "permission-denied"),
  notFound: (msg = "Resource not found.") => new AppError("NOT_FOUND", msg, "not-found"),
  conflict: (msg: string) => new AppError("CONFLICT", msg, "already-exists"),
  validation: (msg: string) => new AppError("VALIDATION_ERROR", msg, "invalid-argument"),
};
